...
class ActionRecognitionApp:
    ...
    def visualize_predictions_LSTM(self, true_seq, predicted_seq):
        """Create a visualization of only true and predicted frames"""
        plt.close('all')  
        fig, axes = plt.subplots(2, 10, figsize=(20, 6))
        
        for i in range(10):
            # Ground truth frames
            axes[0, i].imshow(true_seq[0, i, :, :, 0], cmap='gray')
            axes[0, i].set_title(f"True {i+1}")
            axes[0, i].axis('off')
            
            # Predicted frames
            axes[1, i].imshow(predicted_seq[0, i, :, :, 0], cmap='gray')
    ...
    def process_action(self, model_type, action_class, video_file):
        ...
        try:
            ...
            if model_type == 'LSTM':
                ...
                visualization_path = self.visualize_predictions_LSTM(true_seq, predicted_seq)
